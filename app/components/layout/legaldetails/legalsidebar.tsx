...
export function LegalSidebar({ currentId, sidebarData }: LegalSidebarProps) {
  ...
  const defaultSidebar =
    lawData.categories.Veritas.sections.LegalUpdates?.variants
      ?.VeritasLegalUpdates1?.legalSidebar;
    ...
    <aside className="w-full space-y-8 select-none">
      ...
      {recentPosts && recentPosts.length > 0 && (
        <FadeIn
          ...
          delay={0.15}
          duration={0.6}
          className="rounded-2xl sm:rounded-3xl bg-[#0B0E14] border border-slate-800/80 p-5 sm:p-6 shadow-xl space-y-5 text-left"
        >
