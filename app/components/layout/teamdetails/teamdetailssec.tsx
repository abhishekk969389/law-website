        ...
        <FadeIn direction="up" delay={0.1}>
          <div className="bg-[#070A11] rounded-3xl shadow-2xl relative overflow-hidden">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
              ...
              {/* LEFT COLUMN: Attorney Header, Bio & 2x2 Info Grid */}
              <div className="lg:col-span-7 flex flex-col justify-between h-full space-y-6">

                <div>
                  {/* Top Tagline Badge: Columns Temple Icon + EXPERIENCED ATTORNEYS + Horizontal Gold Line */}
                  <div className="flex items-center gap-3.5 mb-6">
                    <svg className="w-8 h-8 text-[#D4A359] shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M3 21h18" />
                      <path d="M3 10h18" />
                      <path d="M5 10v11" />
                      <path d="M9 10v11" />
                  ...
                  {/* Main Attorney Name (Large Serif Typography) */}
                ...
                {/* 2x2 Contact & Experience Info Grid */}
                  ...
                  <div className="bg-[#0A0F1D] border border-slate-800/90 hover:border-[#D4A359]/50 rounded-2xl p-4 sm:p-5 flex items-center gap-4 transition-all duration-300 shadow-md group">
                    ...
                    <div>
                      ...
                      {contactInfo?.phoneValue ? (
                        <a
                          href={contactInfo.phoneHref || `tel:${contactInfo.phoneValue.replace(/[^0-9+]/g, '')}`}
                          className="text-white text-sm sm:text-base font-bold hover:text-[#D4A359] transition-colors"
                        >
                          {contactInfo.phoneValue}
                        </a>
                      ) : (
                        <span className="text-white text-sm sm:text-base font-bold">+888 (555) 546-33</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-[#0A0F1D] border border-slate-800/90 hover:border-[#D4A359]/50 rounded-2xl p-4 sm:p-5 flex items-center gap-4 transition-all duration-300 shadow-md group">
                    ...
                    <div className="overflow-hidden">
                      ...
                      {contactInfo?.emailValue ? (
                        <a
                          href={contactInfo.emailHref || `mailto:${contactInfo.emailValue}`}
                          className="text-white text-xs sm:text-sm md:text-base font-bold hover:text-[#D4A359] transition-colors truncate block"
                        >
                          {contactInfo.emailValue}
                        </a>
                      ) : (
                        <span className="text-white text-sm sm:text-base font-bold">Info@Example.Com</span>
                      )}
                    </div>
                  </div>
              ...
              {/* RIGHT COLUMN: Profile Photo Frame with Back Card Panel, Gold Outer Line & Floating Scales Badge */}
              <div className="lg:col-span-5 flex justify-center pt-2 sm:pt-4 lg:pt-0">
                <div className="relative w-full max-w-[420px]">

                  {/* Dark Back Card Container with Dot Matrix Pattern Accent */}
                  <div className="absolute inset-y-3 -right-4 w-full rounded-3xl bg-[#080D16] border border-slate-800/90 shadow-2xl overflow-hidden pointer-events-none hidden sm:block">
                    {/* Decorative Dot Matrix Pattern on Right Edge */}
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 grid grid-cols-3 gap-2 opacity-30">
                      {Array.from({ length: 30 }).map((_, i) => (
                        <span key={i} className="w-1.5 h-1.5 rounded-full bg-slate-400/50" />
                      ))}
                    </div>
                  </div>

                  {/* Gold Outer Line Accent Box (Offset Top-Left to Bottom) */}
                  ...
                  {/* Floating Bottom-Right Gold Badge (Scales of Justice Icon) */}
