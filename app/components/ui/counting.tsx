  ...
  useEffect(() => {
    ...
    return springValue.on("change", (latest) => {
      if (ref.current) {
        ...
        const paddedNum = isPadded
          ? formattedNum.padStart(match[2].length, "0")
          : formattedNum;
...
const defaultCountingData: CountingItem[] =
  lawData.categories.Veritas.sections.Counting?.variants?.VeritasCounting1
    ?.counting || [];
...
export default function Counting({
  ...
  return (
    <FadeIn
      delay={0.1}
      duration={0.6}
      className={`w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 mt-8 sm:mt-10 md:mt-12 lg:mt-14 ${className}`}
    >
      <div className="w-full bg-[#0E121A]/90 border border-white/10 rounded-2xl px-4 py-4 sm:px-6 sm:py-5 backdrop-blur-md shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
        <StaggerContainer
          staggerChildren={0.1}
          delayChildren={0.15}
          className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 lg:gap-0 divide-y md:divide-y-0 lg:divide-x divide-white/10"
        >
          {data.map((item, index) => {
