...
const defaultNavbarData: NavbarData = (rawLawData as GlobalLawData).navbar;
...
export function Navbar({ data = defaultNavbarData }: NavbarProps) {
    ...
    return (
        <header className="w-full bg-[#0B0E14] text-white border-b border-slate-800/80">
                        ...
                        return (
                            ...
                            >
                                ...
                                >
                                    ...
                                    {hasDropdown && (
                                        <ChevronDown className="w-3.5 h-3.5 text-slate-400 group-hover:text-[#D4A359] transition-transform duration-200 group-hover:rotate-180" />
                                    )}
                                ...
                                {hasDropdown && link.dropdownItems && (
                                    <div
                                        className={`absolute left-0 top-full mt-2 w-60 bg-[#0F141D] border border-slate-800 rounded-xl shadow-2xl py-2 transition-all duration-200 ${activeDropdown === link.title
            ...
            {mobileMenuOpen && (
                <div className="lg:hidden bg-[#0B0E14] border-b border-slate-800 px-5 pt-3 pb-6 space-y-4">
                        ...
                        {navLinks.map((link, idx) => (
                            <div key={idx} className="flex flex-col">
                                <Link
                                    ...
                                    className={`py-2 text-base font-medium flex items-center justify-between ${link.isActive
                                            ? "text-[#D4A359]"
                                ...
                                {link.hasDropdown && link.dropdownItems && (
                                    <div className="pl-4 space-y-1.5 border-l border-slate-800 ml-2 py-1">
                                        {link.dropdownItems.map((dropItem, dropIdx) => (
                                            <Link
                                                key={dropIdx}
                                                href={dropItem.href}
                                                onClick={() => setMobileMenuOpen(false)}
                                                className="block text-sm text-slate-400 hover:text-[#D4A359] py-1"
                                            >
                                                {dropItem.title}
                                            </Link>
                                        ))}
