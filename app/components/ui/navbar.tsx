...
export function Navbar({ data = defaultNavbarData }: NavbarProps) {
    ...
    const isPathActive = (linkHref: string, dropdownItems?: { href: string }[]) => {
        if (!pathname) return false;
        if (linkHref === "/") return pathname === "/";
        if (linkHref && pathname === linkHref) return true;
        if (linkHref && linkHref !== "/" && pathname.startsWith(linkHref)) return true;
        if (dropdownItems) {
            return dropdownItems.some(
                (item) => item.href && item.href !== "/" && (pathname === item.href || pathname.startsWith(item.href))
            );
        }
        return false;
    };
    ...
    return (
        <header className="w-full bg-[#0B0E14] text-white border-b border-slate-800/80">
                    ...
                    {navLinks.map((link, idx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                {/* Dropdown Menu with Framer Motion AnimatePresence */}
                                <AnimatePresence>
            ...
            {/* Mobile Dropdown Menu with Framer Motion & Vertical Scrollability */}
            <AnimatePresence>
                {mobileMenuOpen && (
                    <motion.div
                    ...
                    >
                        {/* Mobile Action Buttons (Top of Mobile Menu) */}
                                ...
                                return (
                                    <div key={idx} className="flex flex-col">
                                        {link.hasDropdown ? (
                                            ...
                                            >
                                                <span>{link.title}</span>
                                                <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${isMobileDropdownOpen ? "rotate-180 text-[#D4A359]" : ""}`} />
                                            </button>
                                        ) : (
                                            <Link
                                                href={link.href}
                                        ...
                                        {/* Collapsible Accordion Dropdown Sub-links */}
                                        <AnimatePresence>
                                            {link.hasDropdown && link.dropdownItems && isMobileDropdownOpen && (
                                                <motion.div
                                                    initial={{ height: 0, opacity: 0 }}
                                                    animate={{ height: "auto", opacity: 1 }}
                                                    exit={{ height: 0, opacity: 0 }}
