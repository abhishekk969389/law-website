    ...
    <section className="relative w-full bg-[#0B0E14] text-white mt-8 sm:mt-10 md:mt-12 lg:mt-14 overflow-hidden">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="relative flex items-center w-full">
          ...
          <div className="w-full lg:px-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-4.5">
            {items.map((item, index) => {
              ...
              return (
                <div
                  key={item.id || index}
                  className="bg-[#090C12] border border-slate-800/60 rounded-[18px] p-3 pt-3 pb-6 flex flex-col items-center text-center relative group hover:border-[#D4A359]/40 transition-all duration-300 shadow-xl"
                >
                  {/* Top Image Container */}
                  <div className="relative rounded-[14px] overflow-hidden w-full h-[175px] sm:h-[190px] bg-[#090C12]">
                    <Image
                      ...
                      alt={item.title}
                      fill
                      className="object-cover scale-[1.22] sm:scale-[1.20] transition-transform duration-500 group-hover:scale-130"
                    />
                    {/* Subtle Overlay */}
                  ...
                  <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-[#0C1217] border border-[#D4A359]/35 flex items-center justify-center text-[#D4A359] -mt-7 mb-3.5 z-10 shadow-2xl group-hover:border-[#D4A359]/70 group-hover:scale-105 transition-all">
                    <IconComp className="w-6 h-6 sm:w-7 sm:h-7 text-[#D4A359]" />
                  </div>

                  {/* Card Title */}
                  <h3 className="text-white font-serif font-medium text-lg sm:text-xl mb-2 tracking-tight">
                    {item.title}
                  </h3>
